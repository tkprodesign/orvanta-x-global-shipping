    ...
    <?php elseif ($existingPayment && strtolower((string)($existingPayment['status'] ?? '')) === 'confirmed'): ?>
        ...
        <script>
            function printExceptionInvoice() {
                ...
                printWindow.document.write('<!DOCTYPE html><html><head><title>Exception Invoice | OrvantaX Global Shipping</title><style>' + styles + '</style></head><body><div class="print-wrap"><h1 class="print-title">Exception Payment Invoice</h1>' + invoiceHtml + '</div></body></html>');
